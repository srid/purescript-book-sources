...
This line includes the JavaScript code which is generated by Pulp. We place it at the end of the file to ensure that the relevant elements are on the page before we try to access them. To rebuild the `Main.js` file, Pulp can be used with the `-O` argument. Make sure the `dist` directory exists first:
...
And that's it! Try the user interface out by running `pulp build -O --to dist/Main.js` and then opening the `html/index.html` file in your web browser.
